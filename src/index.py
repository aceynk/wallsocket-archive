...
def get_status(site) -> tuple:
    ...
    try:
        ...
        if '?p=' + site.split('?p=')[1] in content:
            content = content.replace('?p=' + site.split('?p=')[1], '?p=None')
    ...
    for i in [y for x in [z for z in ctx if not z is None] for y in x]:
        if i[0] in log_.keys():
            ...
            for file in os.scandir(os.path.join(__location__,'archive/')):
                ...
                log_2_out += f'{i[0]} ({i[2]}): New archive file created! src/archive/archive{new_id}.txt. This might mean that the urls linking here were updated?\n'
